...
public class DriveBase extends SubsystemBase {
    ...
    private DriveBase() {
        ...
        /* Set the closed and open loop ramps to zero...
        * These seem to persist inside the Talons so aren't reset
        * when the robot code restarts or power is lost.
        */
        leftFrontTalon.configClosedloopRamp(0);
        leftRearTalon.configClosedloopRamp(0);
        rightFrontTalon.configClosedloopRamp(0);
        rightRearTalon.configClosedloopRamp(0);
        
        leftFrontTalon.configOpenloopRamp(0);
        leftRearTalon.configOpenloopRamp(0);
        rightFrontTalon.configOpenloopRamp(0);
        rightRearTalon.configOpenloopRamp(0);
    ...
    public double getLeftEncoderPos(DistanceUnit unit) {
        // The left encoder is reversed
        return -(leftFrontTalon.getSensorCollection().getQuadraturePosition() / unit.encoderTicks);
    }
    
    public double getRightEncoderPos(DistanceUnit unit) {
        return rightFrontTalon.getSensorCollection().getQuadraturePosition() / unit.encoderTicks;
    }
    
    public double getLeftEncoderVel(DistanceUnit unit) {
        // The left encoder is reversed
        return -(leftFrontTalon.getSensorCollection().getQuadratureVelocity() / unit.encoderTicks);
    }
    
    public double getRightEnccoderVel(DistanceUnit unit) {
        return rightFrontTalon.getSensorCollection().getQuadratureVelocity() / unit.encoderTicks;
    }
}
